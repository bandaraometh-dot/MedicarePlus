...
// Fetch doctors (join users)
$doctors = [];
$sql = "SELECT d.id AS doctor_id, u.id AS user_id, u.first_name, u.last_name, u.email, d.specialization, d.fees FROM doctors d LEFT JOIN users u ON d.user_id = u.id ORDER BY d.created_at DESC LIMIT 100";
$res = $conn->query($sql);
if ($res) {
    ...
    <div class="main-content">
        <div class="content-header">
            <div class="header-left">
                ...
                <p>List of registered doctors and their details</p>
            </div>
            <div class="header-right">
                <a class="card-link" href="admin-doctors-create.php">+ Add Doctor</a>
            </div>
        </div>
        ...
        <section class="stats-section">
            <div class="stats-grid">
                <?php if (empty($doctors)): ?>
                    <div class="stat-card">No doctors found.</div>
                <?php else: ?>
                    <?php foreach ($doctors as $d): ?>
                        <div class="stat-card">
                            <div style="display:flex; gap:12px; align-items:center;">
                                <div class="stat-icon"><img src="<?php echo avatar_url($d['first_name'],$d['last_name']); ?>" style="width:36px;height:36px;border-radius:6px;"/></div>
                                <div class="stat-info">
                                    <h3><?php echo esc($d['first_name'].' '.$d['last_name']); ?></h3>
                                    <p><?php echo esc($d['specialization'] ?? ''); ?> — <?php echo esc($d['email'] ?? ''); ?></p>
                                </div>
                                <div style="margin-left:auto; text-align:right;">
                                    <a class="card-link" href="admin-doctors-edit.php?id=<?php echo (int)$d['doctor_id']; ?>">Edit</a>
                                    <a class="card-link" href="admin-doctors-delete.php?id=<?php echo (int)$d['doctor_id']; ?>">Delete</a>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
